...
$deptLabel = !empty($deptNames) ? implode(', ', $deptNames) : 'Not assigned';
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div class="bg-black p-8 text-center">
                        <div class="w-24 h-24 bg-white rounded-full mx-auto flex items-center justify-center text-4xl font-bold text-gray-700">
                            <?= strtoupper(substr($principal['full_name'] ?? 'P', 0, 1)) ?>
                        </div>
                        <h2 class="text-xl font-bold text-white mt-4"><?= htmlspecialchars($principal['full_name'] ?? '') ?></h2>
                        <p class="text-gray-300">Principal</p>
                    </div>
                    <div class="p-6">
                        <div class="space-y-4">
                            <div class="flex items-center gap-3 text-gray-600">
                                ...
                                <span><?= htmlspecialchars($principal['username'] ?? '') ?></span>
                            </div>
                            <div class="flex items-center gap-3 text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path>
                                </svg>
                                <span><?= htmlspecialchars($deptLabel) ?></span>
                            </div>
                            <div class="flex items-center gap-3 text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                                </svg>
                                <span>Member since <?= formatDate($principal['member_since'] ?? date('Y-m-d')) ?></span>
                            </div>
                            <div class="flex items-center gap-3 text-gray-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <form method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="new_username" required minlength="3" value="<?= htmlspecialchars($principal['username'] ?? '') ?>"
