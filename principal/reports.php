...
// Fetch sections scoped to principal's departments
$sections = [];
...
if ($sectionFilter) {
...
} elseif ($syFilter && !empty($principalDeptIds)) {
    $ph = implode(',', array_fill(0, count($principalDeptIds), '?'));
    ...
    ");
    $studentsStmt->execute(array_merge([$syFilter], $principalDeptIds));
    $students = $studentsStmt->fetchAll();
}
...
if (!empty($termIds) && ($sectionFilter || $studentFilter || $syFilter)) {
    ...
    if ($studentFilter) {
        ...
        // Section or all-section report (scoped to departments)
        $sectionWhere = $sectionFilter ? "AND sec.id = ?" : "";
        $sectionParams = $sectionFilter ? [$sectionFilter] : [];

        $deptWhere = '';
        $deptParams = [];
        if (!$sectionFilter && !empty($principalDeptIds)) {
            $ph = implode(',', array_fill(0, count($principalDeptIds), '?'));
            $deptWhere = "AND at.dept_id IN ($ph)";
            $deptParams = $principalDeptIds;
        }
