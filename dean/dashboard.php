...
$deanDeptCodes = $_SESSION['dept_codes'] ?? [];
$deanName = $_SESSION['name'] ?? 'Dean';
$deptLabel = !empty($deanDeptNames) ? implode(', ', $deanDeptNames) : '';

// Get stats for this dean's departments
$stats = ['teachers' => 0, 'sections' => 0, 'assignments' => 0];
if (!empty($deanDeptIds)) {
    $placeholders = implode(',', array_fill(0, count($deanDeptIds), '?'));
...
<main class="main-content lg:ml-64 min-h-screen">
    <div class="bg-white border-b border-gray-200 px-4 sm:px-8 py-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
                <h1 class="text-2xl font-semibold text-gray-800">Welcome, <?= htmlspecialchars($deanName) ?>!</h1>
                <?php if ($deptLabel): ?>
                <p class="text-gray-500 mt-1">Dean of <span class="font-medium text-gray-700"><?= htmlspecialchars($deptLabel) ?></span></p>
                <?php endif; ?>
            </div>
    ...
    <div class="p-4 sm:p-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <p class="text-gray-600 mb-4">Manage teacher-subject assignments for your college department.</p>
            <a href="/dean/teacher_subjects.php" class="inline-block px-4 py-2 bg-black text-white rounded-lg hover:bg-neutral-800 transition">
