...
// Get the dean's assigned departments (array)
$deanDeptIds = $_SESSION['dept_ids'] ?? [];
$deanDeptNames = $_SESSION['dept_names'] ?? [];
$deanDeptLabel = !empty($deanDeptNames) ? implode(', ', $deanDeptNames) : 'All Departments';

// If dean has no department assigned, show a warning
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        // Verify section belongs to dean's departments
        if (!empty($deanDeptIds)) {
            $ph = deptPlaceholders($deanDeptIds);
            ...
            $checkDept->execute(array_merge([$section_id], $deanDeptIds));
            if (!$checkDept->fetch()) {
...
// Fetch filter options — scoped to dean's departments
if (!empty($deanDeptIds)) {
    $ph = deptPlaceholders($deanDeptIds);
    ...
    // Get teachers who have assignments in these departments' sections
...
// For the add modal, always show all teachers so dean can assign new teachers
$allTeachers = db()->query("SELECT * FROM tbl_teacher ORDER BY name")->fetchAll();

// Build query with filters — always scoped to dean's departments
$params = [];
$whereConditions = [];
...
if (!empty($deanDeptIds)) {
    ...
    $params = array_merge($params, $deanDeptIds);
}
    ...
    <div class="bg-white border-b border-gray-200 px-4 sm:px-8 py-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Department(s): <span class="font-medium text-gray-700"><?= htmlspecialchars($deanDeptLabel) ?></span></p>
                <?php endif; ?>
            </div>
