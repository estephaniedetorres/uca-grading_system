...
// Fetch sections scoped to dean's departments
$sections = [];
if (!empty($deanDeptIds)) {
    $ph = implode(',', array_fill(0, count($deanDeptIds), '?'));
    $sections = db()->prepare("SELECT s.*, at.code as track_code, at.`desc` as track_name FROM tbl_section s JOIN tbl_academic_track at ON s.academic_track_id = at.id WHERE at.dept_id IN ($ph) AND s.status = 'active' ORDER BY s.section_code");
    $sections->execute($deanDeptIds);
    $sections = $sections->fetchAll();
}

// Get students based on section filter (scoped to departments)
$students = [];
if ($sectionFilter) {
...
} elseif ($syFilter && !empty($deanDeptIds)) {
    $ph = implode(',', array_fill(0, count($deanDeptIds), '?'));
    $studentsStmt = db()->prepare("
        ...
        WHERE e.sy_id = ? AND e.status = 'enrolled' AND at.dept_id IN ($ph)
        ORDER BY s.last_name, s.given_name
    ");
    $studentsStmt->execute(array_merge([$syFilter], $deanDeptIds));
    $students = $studentsStmt->fetchAll();
}
...
if (!empty($termIds) && ($sectionFilter || $studentFilter || $syFilter)) {
    ...
    if ($studentFilter) {
        ...
        $deptWhere = '';
        $deptParams = [];
        if (!$sectionFilter && !empty($deanDeptIds)) {
            $ph = implode(',', array_fill(0, count($deanDeptIds), '?'));
            $deptWhere = "AND at.dept_id IN ($ph)";
            $deptParams = $deanDeptIds;
        }
        ...
        $sql = "
            ...
            LEFT JOIN tbl_grades g ON g.enroll_id = e.id AND g.term_id IN ($termPlaceholders)
            LEFT JOIN tbl_term t ON g.term_id = t.id
            WHERE e.sy_id = ? AND e.status != 'dropped' $sectionWhere $deptWhere
            ORDER BY st.last_name, st.given_name, sub.subjcode
        ";
        $params = array_merge($termIds, [$syFilter], $sectionParams, $deptParams);
        $stmt = db()->prepare($sql);
        $stmt->execute($params);
        $grades = $stmt->fetchAll();
        ...
        $enrolledSql = "
            ...
            " . ($sectionFilter ? "AND st.section_id = ?" : $deptWhere) . "
            GROUP BY e.student_id
        ";
        $enrolledParams = [$syFilter];
        if ($sectionFilter) {
            $enrolledParams[] = $sectionFilter;
        } else {
            $enrolledParams = array_merge($enrolledParams, $deptParams);
        }
...
// Build individual student: group by subject
$gradesBySubject = [];
...
include '../includes/sidebar_dean.php';
?>
...
<main class="main-content lg:ml-64 min-h-screen">
    ...
    <div class="p-4 sm:p-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden print-area">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody>
                        <?php
                        $totalUnits = 0; $weightedSum = 0; $gradeSum = 0; $gradeCount = 0;
                        foreach ($gradesBySubject as $subj):
                            $fg = $subj['final_grade'];
                    ...
                    <tfoot class="bg-gray-50">
                        <tr class="border-t-2 border-gray-200">
                            <?php $allSubjectsGraded = ($gradeCount >= $totalEnrolled && $totalEnrolled > 0); ?>
                            <?php if ($isReportCollege): ?>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
            ...
            <p class="text-gray-500">No grade records found for the selected filters.</p>
        </div>
