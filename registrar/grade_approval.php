        ...
        switch ($action) {
            case 'approve':
                ...
                if ($oldData && $oldData['status'] === 'submitted') {
                    // Approve and finalize in one step
                    $stmt = db()->prepare("
...
// Helper function to log grade history
...
// Send email notification to parent/guardian when grade is approved
...
$statusCounts = db()->query("
    SELECT status, COUNT(*) as count FROM tbl_grades GROUP BY status
                            ...
                            <td class="px-4 py-4 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    ...
                                    <span class="text-gray-400 text-sm">
                                        <?php if ($statusFilter === 'finalized'): ?>
                                        <span class="text-xs text-gray-500">
                                            <?= $grade['finalized_at'] ? date('M j, Y', strtotime($grade['finalized_at'])) : '-' ?>
                                        </span>
                                        <?php else: ?>
                                        -
                                        <?php endif; ?>
...
// Grade approval modal functions (named differently to avoid conflict with footer.php)
