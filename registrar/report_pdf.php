...
if ($studentFilter) {
    ...
    // All sections for this SY — get students from sections assigned to this SY
    $stStmt = db()->prepare("
        SELECT st.*, sec.section_code, at.code as course_code, at.`desc` as course_name, d.code as dept_code, lv.code as level_code
        FROM tbl_student st
        JOIN tbl_section sec ON st.section_id = sec.id
...
class GradeReportPDF extends FPDF {
    ...
    // Helper: get number of lines a text would occupy in a given width
    ...
    // Draw a table row with text wrapping — cells auto-expand to tallest cell
    ...
    function GradeTableStandard($grades, $isCollege, $totalEnrolled) {
        // Group by subject to remove duplicates (multiple grading periods per subject)
        $subjects = [];
        foreach ($grades as $g) {
            $code = $g['subjcode'];
        ...
        // Portrait columns: No | Code | Description | Teacher | Units | Final Grade | Remarks = 190mm
...
// Create PDF - Portrait A4, 2 students per page
...
foreach ($studentsList as $student) {
    ...
    // Add page for 1st student on each page (every 2 students)
    if ($studentIndex % 2 === 0) {
        $pdf->AddPage();
    } else {
        // Separator line between the two students
        $pdf->Ln(3);
